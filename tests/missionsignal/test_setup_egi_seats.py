...
def test_prints_one_link_and_promotes_users_to_founding():
    ...
    assert text.count("/invite/") == 1              # one shared link
    assert "first person to open it becomes the account admin" in text
    assert "is_admin" in text                        # note on switching the admin
