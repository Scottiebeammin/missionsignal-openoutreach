...
- **Opportunity Lifecycle V1** — Deterministic opportunity management layer over the manually managed Opportunity inventory. `/projects/<id>/pipeline/` groups opportunities by lifecycle stage, Discovery shows compact lifecycle badges and expanded lifecycle detail panels, Matching shows suggested lifecycle stage defaults, Dashboard shows pipeline summary counts, and EcosystemSignal shows Lifecycle Health. It does not add drag-and-drop, collaboration, AI, agents, crawling, APIs, external databases, or live discovery.
...
- **`signals/lifecycle.py`** — Deterministic Opportunity Lifecycle V1 helper. It defines lifecycle stage order, stage-specific recommended next steps, active/submitted/awarded lifecycle health counts, and grouped board columns for the pipeline workspace. It reads local `Opportunity` rows only and performs no automation or external calls.
