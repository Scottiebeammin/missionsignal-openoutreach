...
- **Anansi Atlas Hosting Notes** — `docs/anansi_atlas_hosting.md` records the observed `anansiatlas.com` DNS state, production Django email variables, and deployment checklist for host, database, static files, allowed hosts, DNS records, and SSL. No DNS, deployment, or secret changes are performed by the app.
