...
- **Demo Recording Readiness V1** — Keeps deployment paused while making the demo path camera-ready. The Executive Dashboard includes a lightweight Start Here flow through Opportunity Web, Snapshot, Pathways, Readiness, Relationships, and Pipeline; deterministic BridgeForward demo records use polished document/evidence notes; recording support lives in `docs/anansi_atlas_demo_script.md`. No DNS, deployment, scoring, matching, discovery, model, route, or major capability changes are introduced.
- **Anansi Atlas Hosting Notes / Production Prep V1** — `docs/anansi_atlas_hosting.md` records the observed `anansiatlas.com` DNS state, production Django/email variables, recommended deployment target, current-host questions, and live-site requirements. `docs/anansi_atlas_deployment_checklist.md` and `docs/render_deployment_steps.md` provide the go-live and Render setup checklists. Settings remain local-first but support environment-driven `SECRET_KEY`, `DEBUG`, `ALLOWED_HOSTS`, `CSRF_TRUSTED_ORIGINS`, Django email settings, and optional PostgreSQL `DATABASE_URL`; `openoutreach/wsgi.py` and production `gunicorn` support Docker web deployment. No DNS, deployment, or secret changes are performed by the app.
