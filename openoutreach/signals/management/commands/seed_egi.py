"""
Seed / refresh the Empowered Girls Inc. (EGI) workspace with researched real-world data.

    python manage.py seed_egi                      # finds (or creates) the EGI org safely
    python manage.py seed_egi --organization-id N  # explicit target

Production-safe rewrite of the retired repo-root ``seed_egi.py`` one-off, which was
written for a single-org demo database and wiped GLOBAL tables. This command:
- never wipes global reference tables (Funder, ResourceProvider, GovernmentEntity,
  SourceOrganization, funding PartnerOrganization) — curated rows are upserted by
  name, so 990-PF-derived funders and other tenants' data survive;
- resets project-scoped rows (opportunities, contacts, partners, celebrations,
  evidence, documents, pilot profile) for the EGI project only, preserving
  ingested opportunities (non-empty external_id, e.g. grants.gov rows);
- only mutates an organization whose name matches "Empowered Girls" (or the one
  passed via --organization-id); otherwise it creates a fresh org + project.
"""
...
def upsert_by_name(model, name, defaults):
    """update_or_create keyed on name, tolerant of pre-existing duplicate names
    (13k+ 990-PF derived funders share the table): updates the oldest match,
    creates when there is none. Curated seed data wins over derived rows."""
    obj = model.objects.filter(name=name).order_by("pk").first()
    if obj is None:
...
class Command(BaseCommand):
    help = "Seed/refresh the Empowered Girls Inc. workspace (project-scoped resets, global tables upserted by name)."

    def add_arguments(self, parser):
        parser.add_argument(
            "--organization-id", type=int, default=None,
            help="Target organization id (required if several orgs match 'Empowered Girls').",
        )

    @transaction.atomic
    def handle(self, *args, **options):
        # ── 2. Update organization profile ────────────────────────────────────
        self.stdout.write("Updating organization to Empowered Girls Inc…")
        ...
        if org_id is not None:
            ...
            candidates = list(Organization.objects.filter(name__icontains="Empowered Girls")[:2])
            if len(candidates) > 1:
                raise CommandError("Multiple organizations match 'Empowered Girls' — pass --organization-id.")
            org = candidates[0] if candidates else Organization()
        ...
        project.save()
        self.stdout.write(f"  Org: {org.name} | Project: {project.name}")

        # Reset ONLY this project's seeded rows. Global reference tables (Funder,
        # ResourceProvider, GovernmentEntity, SourceOrganization, funding partners)
        # are never wiped — they are upserted by name below. Ingested opportunities
        # (non-empty external_id, e.g. grants.gov rows) are preserved.
        self.stdout.write("Resetting EGI project rows…")
        Opportunity.objects.filter(project=project, external_id="").delete()
        RelPartnerOrg.objects.filter(project=project).delete()
        OrganizationContact.objects.filter(project=project).delete()
        Celebration.objects.filter(project=project).delete()
        DocumentVaultItem.objects.filter(project=project).delete()
        EvidenceLibraryItem.objects.filter(project=project).delete()
        PilotProfile.objects.filter(project=project).delete()

        # ── 4. Source Organizations ────────────────────────────────────────────
        self.stdout.write("Seeding source organizations…")
        src_orgs = {}
        src_map = [
        ...
        for name, org_type, website in src_map:
            s = upsert_by_name(SourceOrganization, name, {
                "organization_type": org_type, "website": website,
                "active": True, "last_reviewed_at": now(),
            })
            src_orgs[name] = s
            self.stdout.write(f"  SourceOrg: {name}")

        # ── 5. Funders ────────────────────────────────────────────────────────
        self.stdout.write("Seeding funders…")
        funders_data = [
            {
        ...
        for d in funders_data:
            upsert_by_name(Funder, d["name"], {**{k: v for k, v in d.items() if k != "name"}, "last_reviewed_at": now()})
            self.stdout.write(f"  Funder: {d['name']}")
        ...
        for d in opps_data:
            ...
            self.stdout.write(f"  Opportunity: {d['name']}")

        # ── 7. Funding Partner Organizations ──────────────────────────────────
        self.stdout.write("Seeding funding partner organizations…")
        funding_partners = [
            {
        ...
        for d in funding_partners:
            upsert_by_name(FundingPartnerOrg, d["name"], {**{k: v for k, v in d.items() if k != "name"}, "active": True, "last_reviewed_at": now()})
            self.stdout.write(f"  Funding Partner: {d['name']}")

        # ── 8. Resource Providers ─────────────────────────────────────────────
        self.stdout.write("Seeding resource providers…")
        resources = [
            {
        ...
        for d in resources:
            upsert_by_name(ResourceProvider, d["name"], {k: v for k, v in d.items() if k != "name"})
            self.stdout.write(f"  Resource: {d['name']}")

        # ── 9. Government Entities ────────────────────────────────────────────
        self.stdout.write("Seeding government entities…")
        gov_entities = [
            {
        ...
        for d in gov_entities:
            upsert_by_name(GovernmentEntity, d["name"], {k: v for k, v in d.items() if k != "name"})
            self.stdout.write(f"  Gov Entity: {d['name']}")

        # ── 10. Relationship Partners (signals app) ───────────────────────────
        self.stdout.write("Seeding relationship partners…")
        rel_partners = [
        ...
        self.stdout.write("Seeding organization contacts…")
        contacts = [
        ...
        # ── 12. Celebrations ──────────────────────────────────────────────────
        self.stdout.write("Seeding celebrations…")
        celebrations = [
        ...
        # ── 13. Evidence Library ──────────────────────────────────────────────
        self.stdout.write("Seeding evidence library…")
        evidence = [
        ...
        # ── 14. Document Vault ────────────────────────────────────────────────
        self.stdout.write("Seeding document vault…")
        docs = [
        ...
        # ── 15. Pilot Profile ─────────────────────────────────────────────────
        self.stdout.write("Seeding pilot profile…")
        ...
        self.stdout.write("  Empowered Girls Inc. workspace is seeded")
        self.stdout.write("=" * 55)
        self.stdout.write(f"  Opportunities:    {Opportunity.objects.filter(project=project).count()}")
        self.stdout.write(f"  Funders:          {Funder.objects.count()}")
        self.stdout.write(f"  Funding Partners: {FundingPartnerOrg.objects.count()}")
        self.stdout.write(f"  Resources:        {ResourceProvider.objects.count()}")
        self.stdout.write(f"  Gov Entities:     {GovernmentEntity.objects.count()}")
        self.stdout.write(f"  Rel Partners:     {RelPartnerOrg.objects.filter(project=project).count()}")
        self.stdout.write(f"  Contacts:         {OrganizationContact.objects.filter(project=project).count()}")
        self.stdout.write(f"  Celebrations:     {Celebration.objects.filter(project=project).count()}")
        self.stdout.write(f"  Evidence:         {EvidenceLibraryItem.objects.filter(project=project).count()}")
        self.stdout.write(f"  Documents:        {DocumentVaultItem.objects.filter(project=project).count()}")
        self.stdout.write(f"  Pilot Profiles:   {PilotProfile.objects.filter(project=project).count()}")
        self.stdout.write("")
